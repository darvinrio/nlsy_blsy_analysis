...
def savePlot(dataFile):
    ...
    y = []
    err = []
    with open('../data/BLSy_source/'+dataFile,'r') as csvfile:
    ...
    plot.xlabel('MJD')
    plot.ylabel('V-Mag')
    plot.title('SSDS_Name '+dataFile[0:15])
    plot.savefig('../BLSy_plots/'+dataFile[0:15]+'.png')
    plot.clf()

sourcesFile=open('../data/BLSy_sourceList.txt','r')
